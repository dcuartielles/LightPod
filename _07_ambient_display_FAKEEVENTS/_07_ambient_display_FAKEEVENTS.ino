/* 07 Ambient display of (LED) rings
    ...
    (this is the so-called John Malkovich version, because it is not 7, but it is also not 8)
...
void loop() {
  ...
  // timers
  if (millis() - timerEvent > 30000 && !runEvent) {
    float r = random(10);
    if (r > 5) {
      timerSound = millis();
      runEvent = true;
      interaction = WAVE_INV;
      Serial.print("event val: "); Serial.println(r);
    }
  }

  if (millis() - timerSound > 10000 && runEvent) {
    timerEvent = millis();
    runEvent = false;
    interaction = NETWORK_SEARCH;
    fadeOutRings();
...
void fadeOutRings() {
  int val = innerRing.getPixel(0).r;
  while (val > 0) {
    val--;
    if (val < 0) val = 0;
    innerRing.setRing(Pixel(val, val, val));
    updateRings();
    pixels.show();
    delay(10);
  }
  val = midRing.getPixel(0).r;
  while (val > 0) {
    val--;
    if (val < 0) val = 0;
    midRing.setRing(Pixel(val, val, val));
    updateRings();
    pixels.show();
    delay(10);
  }
  val = outerRing.getPixel(0).r;
  while (val > 0) {
    val--;
    if (val < 0) val = 0;
    outerRing.setRing(Pixel(val, val, val));
    updateRings();
    pixels.show();
    delay(10);
  }
